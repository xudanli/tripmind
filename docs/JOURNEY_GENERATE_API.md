...
9. **目的地推荐**：如果不提供 `destination`，系统会根据 `intent`、`preferences.interests` 等信息自动推荐目的地。建议至少提供 `intent` 或 `preferences.interests` 之一

10. **意图信息利用**：提供 `intent` 字段可以显著提升行程生成的质量和相关性，建议在可能的情况下提供
